...
let graphql_query ~access_token query variables =
  ...
  in
  let headers =
    Header.init_with "Authorization" ("token " ^ access_token)
  in
      ...
      | `List errors ->
         raise
           (GraphQL_Failure
              (List.map
                 errors
                 ~f:(fun error -> error |> member "message" |> to_string
                 )
              )
           )
...
let string_match ~regexp string =
  try let _ = Str.search_forward (Str.regexp regexp) string 0 in true
  ...
  struct
    ...
    let get_backport_info bot_name { description } =
      ...
      if string_match ~regexp description then
        ...
        in
        let rejected_milestone = Str.matched_group 4 description
        in
        Some
          { backport_to
...
module ProjectCard =
  struct
    type t =
      { id : string
      ; column : ProjectColumn.t
      ; columns : ProjectColumn.t list
      }
