...
let fetch_bench_results ~job_info () =
  ...
  in
  let* summary_table = get_table "bench_summary.txt" in
  let* slow_table = get_table "slow_table.txt" in
  let* fast_table = get_table "fast_table.txt" in
  let* slow_table_pdiff = get_table "slow_table_pdiff.txt" in
  let* fast_table_pdiff = get_table "fast_table_pdiff.txt" in
  Lwt_result.return
    BenchResults.
...
let bench_text results =
  ...
  ; pp_table "slow_downs" slow_table ~emoji:":turtle:"
...
let bench_comment ~bot_info ~owner ~repo ~number ~gitlab_url
  ...
  let message =
    ...
    ; pp_table_details "slow_downs" slow_table ~emoji:":turtle:"
