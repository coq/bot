...
let milestone_edited_action ~bot_info ~owner ~repo ~number
    ~request_inclusion_column =
  let maybe_add_pr_to_column {pr_id; pr_number; cards} =
    if
      cards
      |> List.concat_map ~f:(fun {columns} ->
             columns
             |> List.filter_map ~f:(fun column ->
                    column.GitHub_GraphQL.Queries.Column.databaseId ) )
      |> List.exists ~f:(Int.equal request_inclusion_column)
    then
      Lwt_io.printf
        "Ignoring PR with ID %s because it is already in the backporting \
         project.\n"
        (GitHub_GraphQL.ID.serialize pr_id)
    else
      GitHub_mutations.add_pr_to_column ~bot_info ~pr_id:pr_number
        ~column_id:request_inclusion_column
  in
  GitHub_queries.get_milestone_merged_prs ~bot_info ~owner ~repo ~number
  >>= function
  | Ok prs ->
      Lwt_list.iter_p maybe_add_pr_to_column prs
  | Error err ->
      Lwt_io.printf "Error while looking for milestone merged PRs: %s\n" err
