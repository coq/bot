...
let callback _conn req body =
  ...
  | "/push" | "/pull_request" (* legacy endpoints *) | "/github" -> (
      ...
      | Ok (_, MilestoneEdited ({owner; name= repo}, {number; description}))
        -> (
        match
          description
          |> Option.bind ~f:(GitHub_queries.extract_backport_info ~bot_info)
        with
        | None | Some {backport_info= []} ->
            Server.respond_string ~status:`OK
              ~body:
                (f
                   "Milestone %d of project %s/%s was edited but does not \
                    contain backport info. Doing nothing."
                   number owner repo )
              ()
        | Some {backport_info= [{request_inclusion_column}]} ->
            (fun () ->
              action_as_github_app ~bot_info ~key ~app_id ~owner ~repo
                (milestone_edited_action ~owner ~repo ~number
                   ~request_inclusion_column ) )
            |> Lwt.async ;
            Server.respond_string ~status:`OK
              ~body:
                (f
                   "Milestone %d of project %s/%s was edited with backporting \
                    instructions: adding merged PRs to request inclusion \
                    column."
                   number owner repo )
              ()
        | Some {backport_info= _ :: _ :: _} ->
            Server.respond_string ~status:`OK
              ~body:
                (f
                   "Milestone %d of project %s/%s was edited with multiple \
                    backporting instructions: no automatic management of \
                    merged PRs in this case."
                   number owner repo )
              () )
