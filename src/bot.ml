...
let github_endpoint req body =
  let extract_minimize_file arg =
    arg
  ...
  | Ok (_, PullRequestUpdated (PullRequestClosed, pr_info)) ->
      (fun () ->
        ...
        action_as_github_app ~bot_info ~key ~app_id
          ...
          (pull_request_closed_action ~gitlab_mapping ~github_mapping pr_info)
        )
      |> Lwt.async ;
  ...
  | Ok (signed, CommentCreated comment_info) -> (
      ...
      | None ->
          ...
          then (
            ...
            string_match ~regexp:(f "@%s:? [Bb]ench" @@ Str.quote bot_name) body
            && comment_info.issue.pull_request
            && String.equal comment_info.issue.issue.owner "coq"
            && String.equal comment_info.issue.issue.repo "coq"
            && signed
  ...
  | Ok (signed, CheckRunReRequested {external_id}) ->
      if not signed then
        ...
        Server.respond_string ~status:`OK
          ~body:
            (f "Received a request to re-run a job / pipeline (GitLab ID : %s)."
               external_id )
          () )
