...
let callback _conn req body =
  ...
  | "/check-stale-pr" ->
      ...
      | [owner; repo] ->
        ...
        let throttle = 10 in
        (fun () ->
        action_as_github_app ~bot_info ~key ~app_id ~owner ~repo (coq_check_needs_rebase_pr ~owner ~repo ~warn_after ~close_after ~throttle) >>= fun () ->
        action_as_github_app ~bot_info ~key ~app_id ~owner ~repo (coq_check_stale_pr ~owner ~repo ~after:close_after)) |>          Lwt.async;
